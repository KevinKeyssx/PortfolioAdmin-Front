<script lang="ts">
  ...
  import { updateListaValor, deleteListaValor, lovValoresStore } from '../lib/store';
...
>
  <!-- Top accent line -->
  <div class="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-primary to-transparent opacity-0 transition-opacity duration-500 {selected ? 'opacity-100' : 'group-hover:opacity-60'}"></div>

  <!-- Glow effect when selected -->
  {#if selected}
    <div class="absolute inset-0 rounded-2xl bg-gradient-to-br from-primary/15 via-transparent to-accent/8 animate-pulse-glow -z-10"></div>
  {/if}

  <div class="flex items-start gap-4">
    <!-- Icon -->
    <div class="flex-shrink-0 w-12 h-12 rounded-xl bg-gradient-to-br from-primary/30 to-accent/15 flex items-center justify-center group-hover:from-primary/40 group-hover:to-accent/25 transition-all duration-300 shadow-md shadow-primary/10">
    ...
    <div class="flex-1 min-w-0">
      <div class="flex items-center gap-3">
        ...
        <span class="px-2.5 py-0.5 text-xs font-semibold rounded-full bg-gradient-to-r from-accent/25 to-accent/15 text-accent-light border border-accent/20">
          {itemCount} items
        </span>
      </div>
      {#if lista.comment}
  ...
  <div class="mt-4 pt-3 border-t border-border/30 flex items-center justify-between">
    ...
    <div class="flex items-center gap-2" onclick={(e) => e.stopPropagation()}>
      <span class="text-xs text-text-muted">Activo</span>
