<script lang="ts">
  ...
  import { selectedLista, selectedLovValores, selectedListaIdStore } from '../lib/store';
  ...
  function handleNewLov() {
    editingLov = null;
    showForm = true;
  }

  function handleEdit(lov: LovValor) {
    editingLov = lov;
    showForm = true;
  }
...
<main class="flex-1 h-full flex flex-col overflow-hidden">
  {#if $selectedLista}
    <!-- Header -->
    <div class="p-6 border-b border-border/30 glass">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
          <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-primary via-primary-dark to-accent flex items-center justify-center shadow-lg shadow-primary/40 ring-2 ring-primary/20">
            <span class="text-2xl font-bold text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]">{$selectedLista.description.charAt(0)}</span>
          </div>
          <div>
            <h1 class="text-2xl font-bold text-text">{$selectedLista.description}</h1>
            <p class="text-sm text-text-muted mt-0.5">{$selectedLovValores.length} valores registrados</p>
          </div>
        </div>
        <button
          onclick={handleNewLov}
          class="flex items-center gap-2 px-5 py-3 text-white bg-gradient-to-r from-primary via-primary-dark to-primary rounded-xl shadow-lg shadow-primary/30 hover:shadow-primary/50 hover:shadow-xl hover:scale-105 transition-all duration-300 font-medium ring-1 ring-primary-light/20"
        >
    ...
    <div class="flex-1 overflow-y-auto p-6">
      ...
      {:else}
        <div class="text-center py-16">
          <div class="w-20 h-20 mx-auto mb-6 rounded-full bg-gradient-to-br from-primary/20 to-accent/10 flex items-center justify-center">
    ...
    <div class="flex-1 flex items-center justify-center p-8">
      <div class="text-center max-w-md">
        <div class="w-32 h-32 mx-auto mb-8 relative">
          <div class="absolute inset-0 rounded-full bg-gradient-to-br from-primary/20 to-accent/10 animate-pulse"></div>
          <div class="absolute inset-4 rounded-full bg-surface flex items-center justify-center">
        ...
        <h2 class="text-2xl font-bold text-text mb-3">Selecciona una Lista</h2>
        <p class="text-text-muted leading-relaxed">
          Elige una lista de valores del panel izquierdo para ver y gestionar sus valores asociados.
        </p>
      </div>
    </div>
  {/if}
