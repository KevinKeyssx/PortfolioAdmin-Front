<script lang="ts">
  ...
  let { tags = $bindable([]), placeholder = 'Agregar...', ontags }: Props = $props();
...
<div class="flex flex-wrap gap-2 p-3 bg-surface rounded-xl border border-border/50 focus-within:border-primary/50 focus-within:ring-2 focus-within:ring-primary/20 transition-all duration-300">
  {#each tags as tag, index}
    <span 
      class="inline-flex items-center gap-1 px-3 py-1 bg-gradient-to-r from-primary/20 to-accent/20 text-primary-light rounded-full text-sm animate-scale-in"
    >
      {tag}
      <button
        type="button"
